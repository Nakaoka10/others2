Attribute VB_Name = "AllTrackCsvAggregation_v7_debug"
Option Explicit
...
Private gHeaderDebug As Object
Private gFileDebug As Collection
Private gTotalDataRows As Long
Private gBlankCourseTitleRows As Long
Private gExpectedTrackMismatchRows As Long
Private gBlankUserKeyRows As Long
Private gAcceptedUserRows As Long
Private gCompletedRows As Long

' ============================================================
' Setup
' ============================================================

Public Sub SetupCsvAggregation_v7_Debug()
...
Public Sub RunCsvAggregation_v7_Debug()
    ...
    Set gHeaderDebug = NewDictionary()
    Set gFileDebug = New Collection
    gTotalDataRows = 0
    gBlankCourseTitleRows = 0
    gExpectedTrackMismatchRows = 0
    gBlankUserKeyRows = 0
    gAcceptedUserRows = 0
    gCompletedRows = 0
    ...
    MsgBox _
        ...
        "Accepted User Rows: " & gAcceptedUserRows & vbCrLf & _
        "Completed Rows: " & gCompletedRows & vbCrLf & vbCrLf & _
        "See CsvAggregation_Debug for headers, course titles, and exclusion reasons.", _
        vbInformation
...
Private Function ProcessCsvFile( _
    ...
    Dim fileStage As String
    Dim fileRowCount As Long
    Dim fileBlankCourseCount As Long
    Dim fileMatchedCourseCount As Long
    Dim fileExpectedTrackCount As Long
    Dim fileBlankUserKeyCount As Long
    Dim fileAcceptedCount As Long
    Dim fileCompletedCount As Long
    Dim headerFieldCount As Long
    Dim firstDataFieldCount As Long
    Dim firstDataCourseTitle As String
    Dim firstDataEmail As String

    On Error GoTo FileError

    fileStage = "Opening file"
    fileNo = FreeFile
    Open filePath For Input As #fileNo

    If EOF(fileNo) Then
        fileStage = "Empty file"
        GoTo FileError
    End If

    fileStage = "Reading header"
    lineText = ReadCsvRecord(fileNo)
    fields = ParseCsvLine(lineText)
    headerFieldCount = FieldCount(fields)
    Set headers = BuildHeaderMap(fields)

    RecordHeaderDebug filePath, fields, headers

    If Not headers.Exists(LCase$("Course title")) Then
        fileStage = "Required header not found: Course title"
        GoTo FileError
    End If

    fileStage = "Reading data rows"
    Do While Not EOF(fileNo)

        lineText = ReadCsvRecord(fileNo)

        If Len(lineText) = 0 And EOF(fileNo) Then Exit Do

        fields = ParseCsvLine(lineText)

        fileRowCount = fileRowCount + 1
        gTotalDataRows = gTotalDataRows + 1

        If fileRowCount = 1 Then
            firstDataFieldCount = FieldCount(fields)
            firstDataCourseTitle = CsvField(fields, headers, "Course title")
            firstDataEmail = CsvField(fields, headers, "Email")
        End If

        courseTitle = CsvField(fields, headers, "Course title")

        If Len(courseTitle) = 0 Then
            fileBlankCourseCount = fileBlankCourseCount + 1
            gBlankCourseTitleRows = gBlankCourseTitleRows + 1
            GoTo NextLine
        End If

        RecordCourseTitleDebug courseTitle, courseMap.Exists(NormalizeCourseKey(courseTitle))

        If Not courseMap.Exists(NormalizeCourseKey(courseTitle)) Then
            gUnmatchedCourseRows = gUnmatchedCourseRows + 1
            GoTo NextLine
        End If

        gMatchedExamRows = gMatchedExamRows + 1
        fileMatchedCourseCount = fileMatchedCourseCount + 1
        trackName = CStr(courseMap(NormalizeCourseKey(courseTitle)))

        If StrComp(trackName, expectedTrack, vbTextCompare) <> 0 Then
            gExpectedTrackMismatchRows = gExpectedTrackMismatchRows + 1
            GoTo NextLine
        End If

        fileExpectedTrackCount = fileExpectedTrackCount + 1
        ...
        If Len(courseStatus) = 0 Then
            courseStatus = CsvField(fields, headers, "Course Status")
        End If
        finalScore = CsvField(fields, headers, "Final Score")

        userKey = MakeUserKey(username, email, fullName)
        If Len(userKey) = 0 Then
            fileBlankUserKeyCount = fileBlankUserKeyCount + 1
            gBlankUserKeyRows = gBlankUserKeyRows + 1
            GoTo NextLine
        End If

        fileAcceptedCount = fileAcceptedCount + 1
        gAcceptedUserRows = gAcceptedUserRows + 1
        ...
        If StrComp(courseStatus, "Completed", vbTextCompare) = 0 Then
            ...
            fileCompletedCount = fileCompletedCount + 1
            gCompletedRows = gCompletedRows + 1
...
NextLine:
    ...
    Close #fileNo
    RecordFileDebug filePath, expectedTrack, _
        headerFieldCount, fileRowCount, fileBlankCourseCount, _
        fileMatchedCourseCount, fileExpectedTrackCount, _
        fileBlankUserKeyCount, fileAcceptedCount, fileCompletedCount, _
        "Processed", "First row fields=" & firstDataFieldCount & _
        "; Course title=[" & firstDataCourseTitle & _
        "]; Email=[" & firstDataEmail & "]"
    ProcessCsvFile = True
    Exit Function
...
FileError:
    RecordFileDebug filePath, expectedTrack, _
        ...
        fileMatchedCourseCount, fileExpectedTrackCount, _
        fileBlankUserKeyCount, fileAcceptedCount, fileCompletedCount, _
        "Skipped", fileStage & IIf(Err.Number <> 0, _
...
Private Function NormalizeHeaderName(ByVal headerText As String) As String
    ...
    NormalizeHeaderName = Trim$(s)

End Function


Private Function ReadCsvRecord(ByVal fileNo As Integer) As String
    ...
    Do
        ...
        For i = 1 To Len(recordText)
            ch = Mid$(recordText, i, 1)

            If ch = """" Then
                If i < Len(recordText) And Mid$(recordText, i + 1, 1) = """" Then
                    i = i + 1
                Else
        ...
        If quoteCount Mod 2 = 0 Then Exit Do

    Loop

    ReadCsvRecord = recordText
...
Private Function FieldCount(ByVal fields As Variant) As Long
...
Private Function HeaderColumnNumber( _
    ByVal headers As Object, _
    ByVal headerName As String) As Long

    Dim key As String
    key = LCase$(NormalizeHeaderName(headerName))

    If headers.Exists(key) Then
        HeaderColumnNumber = CLng(headers(key)) + 1
    Else
        HeaderColumnNumber = 0
    End If
End Function

Private Function JoinCsvFieldsForDebug(ByVal fields As Variant) As String
    ...
    For i = LBound(fields) To UBound(fields)
        If Len(s) > 0 Then s = s & " | "
        s = s & "[" & CStr(i + 1) & "] " & CleanText(fields(i))
    Next i

    JoinCsvFieldsForDebug = s
    Exit Function

Failed:
    JoinCsvFieldsForDebug = "(header could not be parsed)"
End Function

Private Sub RecordHeaderDebug( _
    ByVal filePath As String, _
    ByVal fields As Variant, _
    ...
    signature = JoinCsvFieldsForDebug(fields)

    If Not gHeaderDebug.Exists(signature) Then
        info = Array( _
            1, _
            filePath, _
            FieldCount(fields), _
            HeaderColumnNumber(headers, "Username"), _
            HeaderColumnNumber(headers, "Email"), _
            HeaderColumnNumber(headers, "Full Name"), _
            HeaderColumnNumber(headers, "Course title"), _
            HeaderColumnNumber(headers, "Course Status"), _
            HeaderColumnNumber(headers, "Course Enrollment Status"), _
            HeaderColumnNumber(headers, "Final Score"))
        gHeaderDebug.Add signature, info
    Else
        info = gHeaderDebug(signature)
        info(0) = CLng(info(0)) + 1
        gHeaderDebug(signature) = info
    End If
End Sub

Private Sub RecordFileDebug( _
    ByVal filePath As String, _
    ByVal expectedTrack As String, _
    ByVal headerCount As Long, _
    ByVal dataRows As Long, _
    ByVal blankCourseRows As Long, _
    ByVal matchedCourseRows As Long, _
    ByVal expectedTrackRows As Long, _
    ByVal blankUserKeyRows As Long, _
    ByVal acceptedRows As Long, _
    ByVal completedRows As Long, _
    ByVal resultText As String, _
    ByVal detailText As String)

    gFileDebug.Add Array( _
        filePath, expectedTrack, headerCount, dataRows, blankCourseRows, _
        matchedCourseRows, expectedTrackRows, blankUserKeyRows, _
        acceptedRows, completedRows, resultText, detailText)
End Sub

Private Function NormalizeCourseKey(ByVal courseTitle As String) As String

    Dim s As String

    s = CleanText(courseTitle)
    ...
    NormalizeCourseKey = LCase$(Trim$(s))
End Function
...
Private Sub RecordCourseTitleDebug( _
    ...
    key = NormalizeCourseKey(courseTitle)
...
Private Sub WriteCsvAggregationDebug(ByVal courseMap As Object)
    ...
    ws.Range("A1:B1").Value = Array("Diagnostic item", "Count")
    ws.Range("A2:B2").Value = Array("Total data rows read", gTotalDataRows)
    ws.Range("A3:B3").Value = Array("Blank Course title rows", gBlankCourseTitleRows)
    ws.Range("A4:B4").Value = Array("Course title matched to target list", gMatchedExamRows)
    ws.Range("A5:B5").Value = Array("Course title not in target list", gUnmatchedCourseRows)
    ws.Range("A6:B6").Value = Array("Matched course but folder track differed", gExpectedTrackMismatchRows)
    ws.Range("A7:B7").Value = Array("Rows with no Username/Email/Full Name key", gBlankUserKeyRows)
    ws.Range("A8:B8").Value = Array("Rows accepted into user aggregation", gAcceptedUserRows)
    ws.Range("A9:B9").Value = Array("Rows judged Completed", gCompletedRows)

    ws.Range("A12:C12").Value = Array( _
        "Course title in CSV", _
        "Exam Match", _
        "Row Count")

    ws.Rows(1).Font.Bold = True
    ws.Rows(12).Font.Bold = True
    ...
    ws.Range("E12:N12").Value = Array( _
        "Files with header", "First file", "Column count", _
        "Username col", "Email col", "Full Name col", _
        "Course title col", "Course Status col", _
        "Course Enrollment Status col", "Final Score col")
    ws.Rows(12).Font.Bold = True

    rowNo = 13

    For Each key In gHeaderDebug.Keys
        info = gHeaderDebug(CStr(key))
        ws.Cells(rowNo, 5).Resize(1, 10).Value = Array( _
            info(0), info(1), info(2), info(3), info(4), _
            info(5), info(6), info(7), info(8), info(9))
        ws.Cells(rowNo, 15).Value = CStr(key)
        rowNo = rowNo + 1
    Next key

    ws.Cells(12, 15).Value = "Parsed header (1-based positions)"

    ws.Range("Q12:AB12").Value = Array( _
        "File", "Expected track", "Header columns", "Data rows", _
        "Blank Course title", "Matched target course", _
        "Matched expected track", "Blank user key", "Accepted rows", _
        "Completed rows", "Result", "Detail")

    rowNo = 13

    For Each item In gFileDebug
        For c = 0 To 11
            ws.Cells(rowNo, 17 + c).Value = item(c)
        Next c

        If StrComp(CStr(item(10)), "Skipped", vbTextCompare) = 0 Then
            ws.Cells(rowNo, 27).Interior.Color = RGB(255, 192, 0)
        End If
    ...
    If ws.Columns(6).ColumnWidth > 60 Then ws.Columns(6).ColumnWidth = 60
    If ws.Columns(15).ColumnWidth > 80 Then ws.Columns(15).ColumnWidth = 80
    If ws.Columns(17).ColumnWidth > 60 Then ws.Columns(17).ColumnWidth = 60
    If ws.Columns(28).ColumnWidth > 60 Then ws.Columns(28).ColumnWidth = 60
...
Private Function BuildCourseToTrackMap() As Object
    ...
    AddCourse d, "Custom Compiler Basic Layout Design Exam", "WFD-AMS"
    AddCourse d, "Custom Compiler Basic Layout Design", "WFD-AMS"
    AddCourse d, "Custom Compiler: Basic Layout Design", "WFD-AMS"
    AddCourse d, "Custom Compiler: Introduction to Platform Exam", "WFD-AMS"
    AddCourse d, "Custom Compiler Introduction to Platform Exam", "WFD-AMS"
    AddCourse d, "Introduction to Platform", "WFD-AMS"
    AddCourse d, "Custom Compiler: Introduction to Platform", "WFD-AMS"
    AddCourse d, "Custom Compiler: Schematic Entry Exam", "WFD-AMS"
    AddCourse d, "Custom Compiler Schematic Entry Exam", "WFD-AMS"
    AddCourse d, "Schematic Entry", "WFD-AMS"
    AddCourse d, "Custom Compiler: Schematic Entry", "WFD-AMS"
...
Private Sub AddCourse( _
    ...
    d(NormalizeCourseKey(courseTitle)) = trackName
End Sub
...
Private Sub CreateRunButton(ByVal ws As Worksheet)
    ...
    ' Remove old Run shapes/buttons in the top-left area.
    For i = ws.Shapes.Count To 1 Step -1

        Set shp = ws.Shapes(i)

        If StrComp(shp.Name, "RunCsvAggregationShape", vbTextCompare) = 0 Then
            shp.Delete
    ...
    With shp
        .Name = "RunCsvAggregationShape"
        .TextFrame2.TextRange.Text = "Run"
        .OnAction = "'" & ThisWorkbook.Name & "'!RunCsvAggregation_v7_Debug"
