...
Private Function ProcessSourceWorkbook(ByVal filePath As String, ByVal courseMap As Object, _
    ...
    For r = 2 To UBound(data, 1)
        ...
        If Not users.Exists(userKey) Then
            userInfo = Array(username, email, fullName)
...
Private Sub OutputUserTrack(ByVal trackName As String, ByVal users As Object, _
    ...
    WriteUserSummary ws, users.Count
    ...
    totalColumns = 4 + (maxCourseCount * 3)
    ...
    For i = 1 To userOrder.Count
        ...
        For j = 1 To courseOrder.Count
            ...
            outputData(i, c + 1) = courseInfo(1)
            outputData(i, c + 2) = courseInfo(2)
            c = c + 3
        Next j
    Next i

    ws.Cells(HEADER_ROW + 1, 1).Resize(users.Count, totalColumns).Value = outputData
    ApplyDateFormatsToUserSheet ws, maxCourseCount
    FormatSheet ws
End Sub
...
Private Sub OutputPassedTrack(ByVal trackName As String, ByVal passedUsers As Object, _
    ...
    For i = 1 To passedOrder.Count
        ...
        If examCountForUser >= 1 And examCountForUser <= maxExamCount Then
            passedCountBuckets(examCountForUser) = passedCountBuckets(examCountForUser) + 1
        End If
    Next i
    ...
    ws.Cells(HEADER_ROW + 1, 1).Resize(passedUsers.Count, totalColumns).Value = outputData
    ws.Columns(5).NumberFormat = "yyyy/mm/dd"
    ws.Columns(6).NumberFormat = "yyyy/mm/dd"
    FormatSheet ws
End Sub

Private Sub WriteUserSummary(ByVal ws As Worksheet, ByVal totalUsers As Long)
    ws.Range("A1").Value = "Total Users"
    ws.Range("B1").Value = totalUsers
End Sub

Private Sub WritePassedSummary(ByVal ws As Worksheet, ByRef buckets() As Long, ByVal totalPassedUsers As Long)
    Dim i As Long, textSummary As String
    For i = LBound(buckets) To UBound(buckets)
        If Len(textSummary) > 0 Then textSummary = textSummary & " | "
        textSummary = textSummary & CStr(i) & " exam passed: " & CStr(buckets(i))
    Next i
    textSummary = textSummary & " | Total passed users: " & CStr(totalPassedUsers)
    ws.Range("A1").Value = textSummary
End Sub

Private Sub WriteUserHeaders(ByVal ws As Worksheet, ByVal courseCount As Long)
    ...
    For i = 1 To courseCount
        ...
        ws.Cells(HEADER_ROW, c + 1).Value = "Enrollment Date" & i
        ws.Cells(HEADER_ROW, c + 2).Value = "Enrollment End Date" & i
        c = c + 3
    Next i
End Sub

Private Sub WritePassedHeaders(ByVal ws As Worksheet, ByVal examCount As Long)
    Dim i As Long, c As Long
    ws.Cells(HEADER_ROW, 1).Value = "No."
    ws.Cells(HEADER_ROW, 2).Value = "Username"
    ws.Cells(HEADER_ROW, 3).Value = "Email"
    ...
    For i = 1 To examCount
        ws.Cells(HEADER_ROW, c).Value = "Course title" & i
        ws.Cells(HEADER_ROW, c + 1).Value = "Course Enrollment Status" & i
        ws.Cells(HEADER_ROW, c + 2).Value = "Final Score" & i
        c = c + 3
    Next i
End Sub

Private Sub ApplyDateFormatsToUserSheet(ByVal ws As Worksheet, ByVal courseCount As Long)
    Dim i As Long, c As Long
    c = 6
    For i = 1 To courseCount
        ws.Columns(c).NumberFormat = "yyyy/mm/dd"
        ws.Columns(c + 1).NumberFormat = "yyyy/mm/dd"
        c = c + 3
    Next i
End Sub
...
Private Sub FormatSheet(ByVal ws As Worksheet)
    ...
    If lastRow >= HEADER_ROW Then ws.Range(ws.Cells(HEADER_ROW, 1), ws.Cells(lastRow, lastCol)).AutoFilter
    ws.Columns.AutoFit
    For c = 1 To lastCol
