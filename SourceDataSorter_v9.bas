...
Private Sub WriteTrackWorkbook( _
    ...
    outputPath = CombinePath( _
                    trackFolder, _
                    normalizedDate & "_" & trackName & ".csv")

    tempPath = CombinePath( _
                    Environ$("TEMP"), _
                    MakeSafeFileName(normalizedDate & "_" & trackName) & _
    ...
    gStep = "Writing CSV: " & normalizedDate & " / " & trackName
    ...
    For i = 1 To headers.Count
        If i > 1 Then lineText = lineText & ","
        lineText = lineText & CsvEscape(CStr(headers(i)))
    Next i
    ...
    For i = 1 To rows.Count
        ...
        For j = 1 To headers.Count
            ...
            lineText = lineText & CsvEscape(ValueToCsvText(item(j)))
        Next j
    ...
    gStep = "Copying CSV to output folder: " & outputPath
    CopyFileOverwrite tempPath, outputPath

    gStep = "Removing temporary CSV: " & tempPath
...
WriteFail:
    ...
    Err.Raise errNumber, "WriteTrackWorkbook", errDescription
...
Private Function ValueToCsvText(ByVal value As Variant) As String
    ...
    ElseIf IsDate(value) Then
        ValueToCsvText = Format$(CDate(value), "yyyy/mm/dd hh:nn:ss")
    Else
        ValueToCsvText = CStr(value)
    End If
...
Private Sub CopyFileOverwrite(ByVal sourcePath As String, ByVal destinationPath As String)
