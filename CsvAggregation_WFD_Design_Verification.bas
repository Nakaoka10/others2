Attribute VB_Name = "CsvAggregation_DV"
Option Explicit

Private Const SHEET_USER_LIST As String = "DV_UserList"
Private Const SHEET_PASSED_LIST As String = "DV_PassedList"
Private Const CSV_FOLDER_NAME As String = "csv"
Private Const TARGET_SUBFOLDER_NAME As String = "WFD-Design_Verification"
...
Public Sub SetupDVAggregation()
    ...
    MsgBox "DV aggregation sheets were prepared.", vbInformation
End Sub

Public Sub RunDVAggregation()
    ...
    MsgBox "DV aggregation completed." & vbCrLf & _
           "Resolved local folder: " & workbookFolder & vbCrLf & _
           "Date folders processed: " & CStr(dateFolders.Count), vbInformation
    Exit Sub

ErrHandler:
    MsgBox "DV aggregation failed." & vbCrLf & Err.Description, vbExclamation
End Sub
...
Private Sub AddRunButton(ByVal ws As Worksheet)
    ...
    For Each shp In ws.Shapes
        If shp.Name = "btnRunDVAggregation" Then shp.Delete
    Next shp

    Set btn = ws.Buttons.Add(ws.Cells(1, 1).Left, ws.Cells(1, 1).Top, 170, 28)
    btn.Name = "btnRunDVAggregation"
    btn.Caption = "Run DV Aggregation"
    btn.OnAction = "RunDVAggregation"
End Sub
...
Private Function BuildTargetTitleDictionary() As Object
    ...
    BuildTargetTitleDictionary.Add "SystemVerilog Assertions Exam", True
    BuildTargetTitleDictionary.Add "SystemVerilog Verification using UVM Exam", True
    BuildTargetTitleDictionary.Add "SystemVerilog Testbench Exam", True
End Function
