...
Public Sub ValidatePreviousYearAggregation()
    ...
    gStep = "Exporting Validation_Summary.xlsx"

    ExportValidationSummary basePath
...
Private Sub WriteValidationExplanation( _
    ...
    ws.Cells(r, 1).Value = "検証項目"
    ws.Cells(r, 2).Value = "説明"
    ...
    ws.Cells(r, 2).Value = "元のエクセルファイル内で、各トラックの対象Examに登録されている受講者について、同じ人の重複を除いて数えた人数です。"

    r = r + 1
    ws.Cells(r, 1).Value = "Aggregated Users"
    ws.Cells(r, 2).Value = "集計後の各 〇〇_user シートに登録されている受講者について、同じ人の重複を除いて数えた人数です。"

    r = r + 1
    ws.Cells(r, 1).Value = "Missing Users"
    ws.Cells(r, 2).Value = "元データの対象Examには存在するものの、対応する集計後の 〇〇_user シートに存在しない受講者数です。0の場合、受講者の集計漏れは検出されていません。"

    r = r + 1
    ws.Cells(r, 1).Value = "Extra Users"
    ws.Cells(r, 2).Value = "集計後の 〇〇_user シートには存在するものの、元データの対象Exam側では確認できない受講者数です。0の場合、想定外の受講者は検出されていません。"

    r = r + 1
    ws.Cells(r, 1).Value = "Missing Courses"
    ws.Cells(r, 2).Value = "元データには存在する対象Examが、集計後の該当受講者のCourse title一覧から抜けている件数です。0の場合、対象Examの集計漏れは検出されていません。"

    r = r + 1
    ws.Cells(r, 1).Value = "Passed Mismatch"
    ws.Cells(r, 2).Value = "元データで Course Enrollment Status = Completed となっている対象Examのうち、対応する 〇〇_passed シートに存在しない件数です。0の場合、Completedデータの集計漏れは検出されていません。"

    r = r + 1
    ws.Cells(r, 1).Value = "No Exam Access - All Users"
    ws.Cells(r, 2).Value = "元のxlsxファイルには登録されているものの、今回対象としているExamのCourse titleに一度も登場しない人の数です。対象Examへアクセスしていない可能性がある人を確認するための項目です。"

    r = r + 2
    ws.Cells(r, 1).Value = "総合判定"
    ws.Cells(r, 2).Value = "Missing Users、Extra Users、Missing Courses、Passed Mismatch、No Exam Access がすべて0の場合、本マクロによる自動検証では集計結果との不一致は検出されていません。"

    ' Do not wrap text. Use wider columns for readability.
...
Private Sub ExportValidationSummary(ByVal outputFolder As String)
    ...
    Set wsSummary = ThisWorkbook.Worksheets(SHEET_SUMMARY)

    wsSummary.Copy
    Set wbOut = ActiveWorkbook

    ' Remove any buttons or form controls from exported workbook.
    ...
    outputPath = CombinePath(outputFolder, "Validation_Summary.xlsx")

    If FileExistsForValidation(outputPath) Then
        Kill outputPath
    End If
...
ExportError:
    ...
    Err.Raise _
        vbObjectError + 702, _
        "ExportValidationSummary", _
        "Failed to export Validation_Summary.xlsx"

End Sub


Private Function FileExistsForValidation(ByVal filePath As String) As Boolean
    ...
    FileExistsForValidation = fso.FileExists(filePath)
    Exit Function

Failed:

    FileExistsForValidation = False
...
Private Sub FormatValidationSheet(ByVal ws As Worksheet)
    ...
    For c = 1 To lastCol
        If ws.Name = SHEET_SUMMARY And c = 2 Then
            ' Keep the summary explanation column wide.
        ElseIf ws.Columns(c).ColumnWidth > 55 Then
            ws.Columns(c).ColumnWidth = 55
        End If
    Next c

    If ws.Name = SHEET_SUMMARY Then
        ws.Columns(1).ColumnWidth = 28
        ws.Columns(2).ColumnWidth = 110
        ws.Columns(2).WrapText = False
    End If
