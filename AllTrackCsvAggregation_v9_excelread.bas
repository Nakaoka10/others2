...
' v9: Open CSV files with Excel and read cells directly, based on SourceDataSorter.
' v8: Read CSV one physical line at a time; do not merge the entire file into one record.
...
Private Function ProcessCsvFile( _
    ...
    Dim cCourseTitle As Long
    Dim cEnrollmentDate As Long
    Dim cEnrollmentEndDate As Long
    Dim cCourseStatus As Long
    Dim cFinalScore As Long
    ...
    gCurrentStep = "Opening CSV with Excel: " & filePath
    ...
    For c = 1 To lastCol
        ...
        headerText = NormalizeHeaderName( _
                        CleanText(ws.Cells(1, c).Value))

        headerText = Replace(headerText, ChrW(&HFEFF), "")

        If Len(headerText) > 0 Then
            headers(LCase$(headerText)) = c
        End If
    ...
    If headers.Exists(LCase$("Username")) Then
        cUsername = CLng(headers(LCase$("Username")))
    End If

    If headers.Exists(LCase$("Email")) Then
        cEmail = CLng(headers(LCase$("Email")))
    End If

    If headers.Exists(LCase$("Full Name")) Then
        cFullName = CLng(headers(LCase$("Full Name")))
    End If

    cCourseTitle = CLng(headers(LCase$("Course title")))

    If headers.Exists(LCase$("Enrollment Date")) Then
        cEnrollmentDate = CLng(headers(LCase$("Enrollment Date")))
    End If

    If headers.Exists(LCase$("Enrollment End Date")) Then
        cEnrollmentEndDate = CLng(headers(LCase$("Enrollment End Date")))
    End If

    If headers.Exists(LCase$("Course Enrollment Status")) Then
        cCourseStatus = CLng(headers(LCase$("Course Enrollment Status")))
    End If

    If headers.Exists(LCase$("Final Score")) Then
        cFinalScore = CLng(headers(LCase$("Final Score")))
    End If

    For r = 2 To lastRow

        courseTitle = CleanText(ws.Cells(r, cCourseTitle).Value)

        If Len(courseTitle) = 0 Then GoTo NextRow

        RecordCourseTitleDebug _
            courseTitle, _
            courseMap.Exists(LCase$(courseTitle))
        ...
        If cUsername > 0 Then
            ' Read displayed text so leading zeros are preserved.
            username = CleanText(ws.Cells(r, cUsername).Text)
        Else
            username = ""
        End If
        ...
        If cEnrollmentDate > 0 Then
            enrollmentDate = ws.Cells(r, cEnrollmentDate).Value
        Else
            enrollmentDate = ""
        End If

        If cEnrollmentEndDate > 0 Then
            enrollmentEndDate = ws.Cells(r, cEnrollmentEndDate).Value
        Else
            enrollmentEndDate = ""
        End If

        If cCourseStatus > 0 Then
            courseStatus = CleanText(ws.Cells(r, cCourseStatus).Value)
        Else
            courseStatus = ""
        End If

        If cFinalScore > 0 Then
            finalScore = ws.Cells(r, cFinalScore).Value
        Else
            finalScore = ""
        End If
        ...
        If StrComp(courseStatus, "Completed", vbTextCompare) = 0 Then
            ...
            Else
                ...
                ' Later date folders overwrite passed-side dates/scores.
