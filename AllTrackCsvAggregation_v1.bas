...
Public Sub RunCsvAggregation()
    ...
    For i = LBound(dateFolders) To UBound(dateFolders)
        ...
        If IsFolderInPeriod(folderKey, startKey, endKey) Then
            ...
            For t = LBound(tracks) To UBound(tracks)
                ...
                trackFolderPath = CombinePath(dateFolderPath, trackName)
    ...
    MsgBox _
        ...
        "Skipped CSV files: " & skippedFileCount & vbCrLf & vbCrLf & _
        "Six track workbooks were exported beside this macro workbook.", _
        vbInformation
...
Private Function ProcessCsvFile( _
    ...
    Do While Not EOF(fileNo)
        ...
        If Not courseMap.Exists(LCase$(courseTitle)) Then GoTo NextLine
...
Private Function ResolveCsvRoot(ByVal basePath As String) As String
    ...
    candidate = CombinePath(basePath, SORTED_ROOT_FOLDER)
    ...
    If Not IsEmpty(dateFolders) Then
        ResolveCsvRoot = basePath
    End If
End Function
