...
Public Sub RunSourceDataSorter()
    ...
    Dim i As Long, srcDate As String, outDate As String
    ...
    For i = LBound(dateFolders) To UBound(dateFolders)
        srcDate = CStr(dateFolders(i))
        outDate = NormalizeDateFolderName(srcDate)
        gStep = "Sorting date folder: " & srcDate
        SortOneDateFolder CombinePath(basePath, srcDate), CombinePath(outputPath, outDate), outDate
    Next i

    MsgBox "Sorting completed." & vbCrLf & vbCrLf & "Output folder:" & vbCrLf & outputPath, vbInformation
...
Private Sub WriteTrackWorkbook(ByVal trackFolder As String, ByVal normalizedDate As String, ByVal trackName As String, ByVal headers As Collection, ByVal rows As Collection)
    Dim wbOut As Workbook, wsOut As Worksheet
    Dim outputPath As String
    Dim headerData() As Variant, outputData() As Variant, item As Variant
    Dim i As Long, j As Long, usernameCol As Long
    ...
    For i = 1 To headers.Count
        ...
        If StrComp(CStr(headers(i)), "Username", vbTextCompare) = 0 Then usernameCol = i
    Next i
    wsOut.Cells(1, 1).Resize(1, headers.Count).Value = headerData

    If usernameCol > 0 Then wsOut.Columns(usernameCol).NumberFormat = "@"
    ...
    FormatOutputSheet wsOut, headers.Count

    outputPath = CombinePath(trackFolder, normalizedDate & "_" & trackName & ".xlsx")
    If FileExists(outputPath) Then Kill outputPath

    wbOut.SaveAs Filename:=outputPath, FileFormat:=xlOpenXMLWorkbook, CreateBackup:=False
...
Private Function GetSortedDateFolders(ByVal rootPath As String) As Variant
    ...
    For Each subFolder In rootFolder.SubFolders
        If StrComp(CStr(subFolder.Name), OUTPUT_ROOT, vbTextCompare) <> 0 Then
...
Private Function IsDateFolderName(ByVal folderName As String) As Boolean
    ...
    normalized = Replace(folderName, "_", "")
    ...
    If Format$(dt, "yyyymmdd") <> normalized Then Exit Function
...
Private Function NormalizeDateFolderName(ByVal folderName As String) As String
    NormalizeDateFolderName = Replace(folderName, "_", "")
End Function
