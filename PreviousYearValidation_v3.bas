...
Private Sub WriteValidationExplanation( _
    ...
    ws.Cells(r, 2).Value = "元のxlsxファイル内で、各トラックの対象Examに存在するユニークユーザー数です。"

    r = r + 1
    ws.Cells(r, 1).Value = "Aggregated Users"
    ws.Cells(r, 2).Value = "集計後の各 *_user シートに登録されているユニークユーザー数です。"

    r = r + 1
    ws.Cells(r, 1).Value = "Missing Users"
    ws.Cells(r, 2).Value = "元データの対象Examには存在するものの、対応する集計後の *_user シートに存在しないユーザー数です。0の場合、ユーザーの集計漏れは検出されていません。"

    r = r + 1
    ws.Cells(r, 1).Value = "Extra Users"
    ws.Cells(r, 2).Value = "集計後の *_user シートには存在するものの、元データの対象Exam側では確認できないユーザー数です。0の場合、想定外のユーザーは検出されていません。"

    r = r + 1
    ws.Cells(r, 1).Value = "Missing Courses"
    ws.Cells(r, 2).Value = "元データには存在する対象Examが、集計後の該当ユーザーのCourse title一覧から抜けている件数です。0の場合、対象Examの集計漏れは検出されていません。"

    r = r + 1
    ws.Cells(r, 1).Value = "Passed Mismatch"
    ws.Cells(r, 2).Value = "元データで Course Enrollment Status が Completed となっている対象Examのうち、対応する *_passed シートに存在しない件数です。0の場合、Completedデータの集計漏れは検出されていません。"

    r = r + 1
    ws.Cells(r, 1).Value = "No Exam Access - All Users"
    ws.Cells(r, 2).Value = "元のxlsxファイルには存在するものの、定義した対象ExamのCourse titleに一度も登場しないユーザー数です。対象Examへアクセスしていない可能性のあるユーザーを確認するための項目です。"

    r = r + 1
    ws.Cells(r, 1).Value = "No Exam Access - LSTC Students"
    ws.Cells(r, 2).Value = "対象Examに一度も登場しないユーザーのうち、Emailが「8桁の数字@lstc.adip.jp」の形式に一致するLSTC受講者数です。"
