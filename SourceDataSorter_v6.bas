...
Private Sub PrepareSortLog()
    ...
    wsDiscovery.Range("A1:C1").Value = Array("Folder Name", "Folder Path", "Recognition")
    wsDiscovery.Rows(1).Font.Bold = True
    wsDiscovery.Cells(2, 1).Value = "Search Root"
    wsDiscovery.Cells(2, 2).Value = ResolveWorkbookLocalPath(ThisWorkbook.Path)
End Sub
...
Private Sub AddDiscoveryLogRow(ByVal folderName As String, _
    ...
    ws.Cells(nextRow, 2).Value = folderPath
    ws.Cells(nextRow, 3).Value = recognition
End Sub
